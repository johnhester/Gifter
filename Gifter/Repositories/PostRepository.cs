        ...
        public List<Post> GetAll()
{
        ...
        public Post GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT p.Title, p.Caption, p.DateCreated AS PostDateCreated, p.ImageUrl AS PostImageUrl, p.UserProfileId,
                                up.Name, up.Email, up.ImageUrl AS UserProfileImageUrl, up.DateCreated AS ProfileDateCreated
                            FROM Post p
                                LEFT JOIN UserProfile up ON p.UserProfileId = up.Id
                           WHERE p.Id = @Id";
        ...
        public Post GetPostByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                          SELECT 
                            p.Title, p.Caption, p.DateCreated AS PostDateCreated, p.ImageUrl AS PostImageUrl, p.UserProfileId,
        ...
        public List<Post> Hottest(DateTime? since)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@date", since);
